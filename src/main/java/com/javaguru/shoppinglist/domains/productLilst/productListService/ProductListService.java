package com.javaguru.shoppinglist.domains.productLilst.productListService;
...
import com.javaguru.shoppinglist.domains.productLilst.productListRepository.ProductListRepository;
import com.javaguru.shoppinglist.domains.productLilst.productListService.validation.ProductListValidationService;
import com.javaguru.shoppinglist.domains.productLilst.productListService.validation.exceptions.ProductListNotFoundException;
...
public class ProductListService {
    ...
    private ProductListDto getDto(Long userId, Long productId, int quantity) {
    ...
    public ProductListDto save(Long userId, Long productId, int quantity) {
        ProductListDto dto = getDto(userId, productId, quantity);
        productListValidationService.validate(dto);
        ProductListEntity entity = beanMappers.getProductListMapper().toEntity(dto);

        ProductListEntity returnedEntity = productListRepository.getIfPresent(entity.getUser(), entity.getProduct()).orElse(entity);
        ProductListEntity savedEntity;
    ...
    public ProductListDto changeQuantity(Long userId, Long productId, int quantity) {
        ProductListDto dto = getDto(userId, productId, quantity);
        productListValidationService.validate(dto);
        ProductListEntity entity = beanMappers.getProductListMapper().toEntity(dto);
        ProductListEntity returnedEntity = productListRepository.getIfPresent(entity.getUser(), entity.getProduct()).orElse(entity);
        returnedEntity.setQuantity(quantity);
        ProductListEntity savedEntity = productListRepository.changeQuantity(returnedEntity);
        return beanMappers.getProductListMapper().toDto(savedEntity);
    }

    public List<ProductListDto> findByUserId(Long userId) {
        UserDto userDto = userService.findById(userId);
        UserEntity userEntity = beanMappers.getUserMapper().toEntity(userDto);

        List<ProductListEntity> entityList = productListRepository.findByUserId(userEntity).orElseGet(ArrayList::new);
